...
def get_api_key() -> str | None:
    ...
    Returns:
        str | None: The resolved API key, "ollama" for local mode, or None if
...
"""
    ...
    # Custom OpenAI-compatible endpoints (e.g. self-hosted vLLM/LM Studio
    # servers) often don't require a real API key. The OpenAI SDK still
    # requires *some* non-empty string, so fall back to a placeholder rather
    # than letting the client crash with a confusing low-level error.
    if provider == "custom" and not api_key:
        return "not-required"
...
def run_command(command: list[str] | str, extra_args: list[str] | None = None):
    ...
    try:
        ...
        print(f"❌ Error: '{shell_command[0]}' not found. Is it installed and on your PATH?")
        sys.exit(1)
    except subprocess.TimeoutExpired:
        print(f"❌ Error: command timed out: {' '.join(shell_command)}")
        sys.exit(1)
...
def generate_commit_message(staged_changes: str, regenerate: bool = False) -> str:
    ...
    try:
        ...
        if not commit_message.strip():
            print("\n⚠️  The model returned an empty commit message.")
    ...
    while True:
        ...
        match action:
            ...
            case "y" | "yes":
                if not commit_message.strip():
                    print(
                        "\n❌ Commit message is empty. Try 'r' to regenerate or 'e' to edit."
                    )
                    continue
                print("committing...")
